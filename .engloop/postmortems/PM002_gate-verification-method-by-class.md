...
| Time | Event |
|---|---|
| T0 | v1.3.0 gate (PM001) requires MODEL+CORD for every module, including pure components. |
...
- **What failed:** the gate treats "verified" as synonymous with "has a MODEL+CORD." But *how* a module
  ...
  Imposing MODEL+CORD on a pure function is ceremony that verifies nothing and tempts a fake model.
- **Why it failed:** PM001 (rightly) made "ready" a gated, evidence-backed verdict, but expressed the
  evidence as a single uniform checklist rather than one keyed to the ARCH002 module classification
  that already exists.
...
After ARCH002 extraction, the residual vertical is domain behavior, which is exactly the stateful,
...
```
Symptom: The gate marks 100%-unit-covered pure components FAIL for "no MODEL/CORD", and offers no honest pass.
Why #1: Q: Why FAIL despite full coverage?        A: The gate requires MODEL+CORD for every module regardless of kind.
Why #2: Q: Why require MODEL+CORD for pure code?  A: The gate specifies one verification method for all modules.
Why #3: Q: Why one method for all?                A: PM001 expressed evidence as a single uniform checklist.
Why #4: Q: Why uniform, ignoring ARCH002 classes? A: PM001's case never exercised the component/vertical distinction.
...
- **Structural fix (mechanical, class-preventing, verifiable):** the gate assigns the verification
  method **by module class**, using the ARCH002 litmus test as the objective classifier, and adds a
  precondition that closes the escape hatch:
  - **Component** (`components/*`, passes the ARCH002 domain-free litmus test): verified by
    **unit/property tests to ≥95% line & branch**. No MODEL/CORD (a model of domain-free code is
    tautological).
  - **Vertical** (`src/*`, domain behavior): verified by **SEK self-modelling** — a `MODEL` + `CORD` +
  ...
  - **Precondition (anti-escape-hatch):** any **generic/domain-free** code still living in the
    vertical is an **ARCH002 violation and a gate FAIL** — it must be extracted to a component first.
...
- **LEARN001** — Verification is not one-size-fits-all: the *method* (unit/property vs
...
- **LEARN002** — A self-hosting/model-based tool can validate itself **only after** its generic code is
  factored out; the residual domain vertical is the legitimate model-exploration surface. Component
  extraction (ARCH002) is therefore a **precondition** of self-validation, not merely tidiness.
- **LEARN003** — When a gate meets a genuinely different context (here, a self-hosting compiler), expect
...
|---|---|---|---|---|
| RPI001 | Refine `docs/standards.md` Readiness Gate: verification method **by module class** (component → unit/property ≥95%; vertical → SEK self-model MODEL+CORD + generated conformance + ≥95%), plus the "vertical must be domain-only; generic code in the vertical is an ARCH002 violation & FAIL" precondition. Keep ≥95%/conformant/green for all. | full | (this PM) | DONE |
| RPI002 | Rework `commands/speckit.engloopkit.coverage.md`: the Readiness Inventory gains a **Class** column (component/vertical); PASS rules branch on class; a component needs no MODEL/CORD but a vertical module does; add the domain-only precondition check. Update the `COV` template accordingly. | full | (this PM) | DONE |
| RPI003 | Note in `model`/`explore` commands + the `using-sek-to-generate-tests` skill: **don't model pure components** — extract them (ARCH002) and unit/property-test them; model only the domain vertical. Cross-reference PM002. | tiny | (this PM) | DONE |
...
- Architecture: ARCH002 (component boundary, the classifier); builds on PM001 (the gate itself)
