# REFACT001: Ordered EngLoop v2 workflow
...
### Custom agents are the guided workflow

The installed `.github/agents/*.agent.md` files are first-class EngLoopKit product
artifacts, not incidental wrappers around commands. They use the supported VS Code
custom-agent header deliberately:

- explicit `name`, `description`, and `argument-hint` for discoverability;
- `target: vscode` and `user-invocable: true` for the intended interaction surface;
- `disable-model-invocation: true` so a workflow stage is never entered merely because
  another model selected it as a subagent;
- least-privilege `tools`, with an explicit `agents` allowlist whenever the `agent`
  tool is justified and `agents: []` otherwise;
- no pinned `model` unless a measured stage-specific need and tested fallback order
  justify one;
- a deterministic `SessionStart` entry-validation hook backed by the installed,
  versioned EngLoopKit tool; the command body runs the same authoritative check when
  preview hooks are unavailable, so hooks are defense-in-depth rather than hidden
  semantics;
- review-first `handoffs` with `send: false`, preserving human approval between
  stages and at every branch.

Every agent has at least one handoff unless it is an unconditional natural end. Stage
31 is the sole unconditional terminal agent: it returns its validated learning result
to the invoking context and therefore has no static next-stage button. Stages 08 and 30
have conditional terminal outcomes (readiness PASS and no-work respectively), but keep
handoffs for their failure/work branches.

Handoffs encode the ratified graph, not numeric adjacency. In particular, Stage 08
offers no Incident or stewardship handoff; those lanes require independent demand or
capacity. All handoff targets must exist in the same exact installed 13-agent set.
...
`PM001/LEARN001` and `PM001/LEARN003` sources: a gate controls whether a transition is
permitted, not whether unrelated work should be invented or scheduled.

### One hidden EngLoop root

V2 uses one tracked, hidden process root: **`.engloop/`**. This is the repository's
EngLoop equivalent of `.github/` and `.specify/` and contains:

- required explicit configuration at `.engloop/config.json`;
- numbered process memory and its numbering registry;
- Learnings Pyramid subject cards and retrieval cases;
- ignored transient output under `.engloop/out/`.

There is no separate `.engloopkit/` directory and no fallback from `.engloop/` to the
old `engloop/` path. The root `NORTHSTAR.md` and `LEARNINGS.md` remain deliberately
visible because they are human and agent entry points, not implementation detail.

The nomenclature checkpoint renamed the tracked process tree to `.engloop/` with Git
history preserved, moved the initial gathered direction content to `NORTHSTAR.md`, and
removed the empty legacy direction machinery. A repository containing both `engloop/` and `.engloop/` after cutover
is ambiguous and fails validation rather than merging or choosing one.
...
Git is the version history. v2 removes the legacy numbered direction class, counter,
required subdirectory, and template. Existing initial direction records are moved and evolved
...
a numbered `REFACT`. It does **not** emit a new direction snapshot. It normally hands an existing product
...
The complete set of accepted `PMxxx/LEARNxxx` entries, in chronology, is the equivalent
...
Each source identity is globally addressable as the pair `PMxxx/LEARNxxx`, even though
`LEARN` numbering remains local to its post-mortem.

#### Layer 2 — subject index cards

Cards live under:

`.engloop/learnings/cards/<subject>.md`
...
- links to every contributing source learning (`PMxxx/LEARNxxx`);
...
`LEARNINGS.md → subject card → PMxxx/LEARNxxx source`
...
- Release the ordered workflow as **EngLoopKit 1.7.0**. “Ordered EngLoop v2” is an
  internal workflow-generation label, not a package major-version instruction.
- EngLoopKit remains on its 1.x maturity runway until the maintainer explicitly
  authorizes 2.x. Do not infer 2.x from compatibility analysis, command replacement,
  or the word “v2”; a future 2.x decision requires separate explicit authority.
- Keep product/bundle/tool/package identity `engloopkit`. Set the command extension ID
  to `engloop`, because Spec Kit requires command namespace `speckit.<extension-id>`;
  this is the authoritative host contract for the 13 `speckit.engloop.*` identities.
- Do not ship old command aliases.
- Replace existing initial direction content with one root `NORTHSTAR.md`; remove the
  legacy numbered direction template and counter from current consumers while relying on Git for history.
- Rename every current tracked `engloop/` artifact tree to `.engloop/` with history
  preserved; place required config at `.engloop/config.json` and transient output at
  ignored `.engloop/out/`; ship no `.engloopkit/` directory or compatibility fallback.
...
- Render and validate rich VS Code custom-agent headers and branch-aware handoffs in
  every consumer; installation fails if Spec Kit drops, rewrites, or cannot resolve a
  required field or target.
...
Create a full v2 specification from `REFACT001`; plan the clean migration; implement all
