...
`MODEL` (SEK model) **and** a `CORD` (CORD exploration) before a project can be "ready for incidents".
...
1. **It demands model-based testing of code that isn't stateful.** SEK's extracted components
  ...
  Producing a `MODEL`/`CORD` for them is **ceremony, not verification**, and it actively invites the
...
never distinguished the two, and treated 97.5%-unit-covered pure code as `FAIL` for "no MODEL/CORD"
while providing no sensible way to earn the pass.

## The maintainer's resolution (the correct model)

> Factor **all** generic/pure code out into `components/` (ARCH002) and unit/property-test each to
...
This reconciles ARCH002 (the component boundary) with the Readiness Gate: the *method* of verification
...
|---|---|---|---|
| T0 | v1.3.0 gate marks SEK `FAIL` for every module lacking MODEL/CORD, incl. 100%-covered pure components. | — | The design gap. |
| T1 | Maintainer asks whether coverage tests are SEK-generated; answer is hand-written. | — | Gap made explicit. |
| T2 | **Stop treating hand-written unit coverage as if it were model-exploration**, and stop planning MODEL/CORD for pure components. | MIT001 | Prevents fake models being authored to satisfy the gate. |
| T3 | File this incident; hand to PM002 to refine the gate's criteria by module class. | MIT002 | IN002 (this doc). |

## Mitigations applied

- **MIT001** — Paused authoring any `MODEL`/`CORD` for pure/domain-free modules; kept them on
...
- [x] No fake models were checked in to satisfy the gate. Evidence: SEK has only the Turnstile
  `MODEL`/`CORD`; components remain unit-tested.
