---
name: speckit.engloop.61-overlay-remove
description: Remove an installed private ELK overlay and restore prior local hooks
  so the checkout appears never overlaid.
...
hooks:
  SessionStart:
  - type: command
    command: dotnet tool run engloopkit validate agent-entry --stage speckit.engloop.61-overlay-remove
      --root .
    timeout: 30
---
...
`dotnet tool run engloopkit validate agent-entry --stage speckit.engloop.61-overlay-remove --root .`
