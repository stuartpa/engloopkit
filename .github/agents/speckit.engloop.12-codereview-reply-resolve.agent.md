...
description: Publish one exact approved reply and/or resolve one exact current review
  thread through an explicit tracked provider adapter, with revision/principal revalidation
  and durable reconciliation evidence.
argument-hint: --packet <.engloop/out/code-review-response/address/*.json> --operation
  <reply|resolve|reply-and-resolve> --adapter <.engloop/provider-adapters/*.json>
target: vscode
user-invocable: true
...
hooks:
  SessionStart:
  - type: command
    command: dotnet tool run engloopkit validate agent-entry --stage speckit.engloop.12-codereview-reply-resolve
      --root .
    timeout: 30
  UserPromptSubmit:
  - type: command
...
1. Revalidate the Stage 11 packet digest, authenticated acting principal, provider,
   ...
   Require the engine-created `.specify/schemas/code-review-address-receipt.schema.json`
   receipt; a merely schema-correct or subsequently altered packet is ineligible. Require
   an exact clean repository root and a clean refreshed packet (`changedFiles: []` and the
   ...
   must return `.specify/schemas/code-review-provider-inspection.schema.json` with exact
   identities, principal, revisions, thread status, provider head, operation, one match,
   a receipt, and `mutationPerformed: false`. Rejection, malformed output, drift,
   ambiguity, timeout, or any claimed mutation fails closed before an approval exists.
...
The tracked adapter manifest uses `.specify/templates/CODE-REVIEW-PROVIDER-ADAPTER-template.json`
...
ID according to `.specify/schemas/code-review-provider-result.schema.json`. A crash,
timeout, malformed result, or unprovable write becomes
