...
description: Implement an accepted SPEC, REFACT plan, or repair scope under accepted
  architecture; do not select or redesign the refactor plan.
...
hooks:
  SessionStart:
  - type: command
    command: dotnet tool run engloopkit validate agent-entry --stage speckit.engloop.04-refactor
      --root .
    timeout: 30
handoffs:
- label: Model current behavior
  agent: speckit.engloop.05-model
  prompt: Model the accepted architecture-conformant product behavior and its rejection
    semantics.
  send: false
---
