...
description: Propose and, only after explicit user approval, remove the single highest-certainty
  dead-code candidate.
...
hooks:
  SessionStart:
  - type: command
    command: dotnet tool run engloopkit validate agent-entry --stage speckit.engloop.41-deadcode
      --root .
    timeout: 30
---
