...
hooks:
  SessionStart:
  - type: command
    command: dotnet tool run engloopkit validate agent-entry --stage speckit.engloop.03-architect
      --root .
    timeout: 30
handoffs:
- label: Start governed refactor
  agent: speckit.engloop.04-refactor
  prompt: Use the accepted architecture above to run the governed specify → plan →
    tasks → implement loop.
  send: false
---
