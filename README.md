...
> no-ID overlay installation, advisory debugger walkthroughs, publication agents, and governed token-efficiency agents ship as **v1.13.1**. No v2.0 release is implied.
...
A released v1.13.1 artifact set contains three immutable pieces:
...
dotnet tool install engloopkit --version 1.13.1 --add-source <release-dir>
...
$bootstrap = Join-Path $env:LOCALAPPDATA 'EngLoopKit\bootstrap\1.13.1'
...
dotnet tool run engloopkit -- overlay install --mode overlay --root <git-root> `
  --tool-version 1.13.1 --tool-nupkg <release-dir>\engloopkit.1.13.1.nupkg `
  --extension-archive <release-dir>\engloopkit-extension-1.13.1.zip
Pop-Location
```
