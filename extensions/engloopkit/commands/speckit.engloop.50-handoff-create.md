...
description: Capture a concise evidence-backed handoff from the just-completed chat work so another chat window or engineering team can continue without reconstructing context.
...
hooks:
  SessionStart:
    - type: command
      command: dotnet tool run engloopkit validate agent-entry --stage speckit.engloop.50-handoff-create --root .
      timeout: 30
---
