...
description: Upgrade the selected repository's root-local ELK tool and installed ELK extension to the latest verified non-prerelease release, or report that both are already current.
...
hooks:
  SessionStart:
    - type: command
      command: dotnet tool run engloopkit validate agent-entry --stage speckit.engloop.80-upgrade-elk --root .
      timeout: 30
---
