---
name: speckit.engloop.60-overlay-pack
...
hooks:
  SessionStart:
    - type: command
      command: dotnet tool run engloopkit validate agent-entry --stage speckit.engloop.60-overlay-pack --root .
      timeout: 30
handoffs:
...
`dotnet tool run engloopkit validate agent-entry --stage speckit.engloop.60-overlay-pack --root .`
