...
After checking PASS, create the paired structured JSON from
`COV-READINESS-template.json`, including exact module rows and the hashed Cobertura report,
then emit the generic current readiness state:

`dotnet tool run engloopkit -- readiness emit --root . --evidence <.engloop/coverage/COVxxx-readiness.json> --verdict pass`

The emitted `.engloop/readiness/current.json` is invalidated by HEAD, evidence, measured
coverage, or product/config worktree byte changes. The Markdown verdict alone has no
authority to emit readiness.
