...
**vertical → SEK self-model (MDL + CRD + generated conformance)**. But it left the *granularity* of
...
- **Structural fix (mechanical, verifiable):** the gate's **vertical self-model criterion is
  behavior-level**:
  - The vertical is verified by **at least one representative SEK self-model** (`MDL` + `CRD`) whose
...
- **LRN001** — A verification criterion must state the **granularity** at which it is satisfied, or
  it invites theatre on one side and gaps on the other.
- **LRN002** — Model-based testing validates a vertical's **observable behavior end-to-end**, not
