...
```text
actual IDs == expected 18-ID set
count(actual IDs) == 18
count(distinct actual IDs) == 18
...
- All 18 commands are owned by the one `engloop` extension of the `engloopkit` product.
...
### Full 18-agent package/install acceptance
...
3. validate all 18 files against the command-loop shape, common header, exact
...
8. inspect `.specify/extensions/.registry`, installed commands, generated agents, and
   generated prompts;
9. assert 18 exact current entries, each once, and zero old entries;
10. parse source and installed YAML into a `GeneratedSurfaceSemanticComparison` and
   prove all 18 common fields, absence rules, exact tool/subagent policies, exact hook
...
The current command-surface target is 1.10.0. “Ordered EngLoop v2” identifies this workflow
generation only and provides no authority for a 2.x product release. Bundle, extension,
tool, catalog, archive names, and release notes MUST agree on 1.10.0. Catalog SHA-256
values are computed from final immutable artifacts. Rebuilding different bits under
1.10.0 is forbidden. Any 2.x value is a release-blocking error unless a later explicit
