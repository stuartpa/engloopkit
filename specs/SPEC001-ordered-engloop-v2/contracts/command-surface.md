...
| 9 | `speckit.engloop.20-incident` | `commands/speckit.engloop.20-incident.md` | operations | `INxxx` + local `MITxxx` |
| 10 | `speckit.engloop.21-postmortem` | `commands/speckit.engloop.21-postmortem.md` | operations | `PMxxx` + `PMxxx/LEARNxxx` + `RPIxxx` |
| 11 | `speckit.engloop.22-repair` | `commands/speckit.engloop.22-repair.md` | operations | repair obligation + Stage 04/05–08/release/target evidence |
| 12 | `speckit.engloop.30-refactor-scan` | `commands/speckit.engloop.30-refactor-scan.md` | stewardship | one `REFACTxxx`, including no-work decisions |
| 13 | `speckit.engloop.31-learnings-pyramid` | `commands/speckit.engloop.31-learnings-pyramid.md` | stewardship | source-linked cards, root index, instruction, validation/retrieval result |
...
```text
actual IDs == expected 13-ID set
count(actual IDs) == 13
count(distinct actual IDs) == 13
...
- All 13 commands are owned by the one `engloopkit` extension.
...
Before authoring all 13 production headers, a disposable fixture MUST:
...
13 production headers are written. An EngLoopKit-owned alternate generator,
post-processing a partial installation, copying hidden fallback agents, or weakening
this contract is forbidden.

### Full 13-agent package/install acceptance
...
3. validate all 13 files against the command-loop shape, common header, exact
   tool/subagent matrix, entry hook/body gate, and 23-edge handoff table;
...
8. inspect `.specify/extensions/.registry`, installed commands, generated agents, and
   generated prompts;
9. assert 13 exact v2 entries, each once, and zero old entries;
10. parse source and installed YAML into a `GeneratedSurfaceSemanticComparison` and
    prove all 13 common fields, absence rules, exact tool/subagent policies, exact hook
...
The first conforming release is 1.7.0. “Ordered EngLoop v2” identifies this workflow
generation only and provides no authority for a 2.x product release. Bundle, extension,
tool, catalog, archive names, and release notes MUST agree on 1.7.0. Catalog SHA-256
values are computed from final immutable artifacts. Rebuilding different bits under
1.7.0 is forbidden. Any 2.x value is a release-blocking error unless a later explicit
