...
- **Feature ID:** SPEC001
- **Status:** READY FOR TASKS — BLOCKING P0 CANARY REQUIRED
- **Date:** 2026-07-10
- **Classification:** Maintainer-directed evolution refactor
- **Primary authority:** [REFACT001 — Ordered EngLoop v2 workflow](../../.engloop/refactors/REFACT001_ordered-engloop-v2.md)
...
REFACT001 is binding wherever it changes the current workflow. The following existing
constraints remain binding:

- [ARCH001](../../.engloop/architecture/ARCH001_bundle-composition.md): the EngLoopKit
...
- [ARCH002](../../.engloop/architecture/ARCH002_command-loop-contract.md): every command is
  a Trigger · Goal · Actions · Verification · Memory loop with an Artifact root note
  and a Done-when gate.
- [ARCH003](../../.engloop/architecture/ARCH003_numbering-memory.md): numbered durable
  artifacts remain monotonic and never reused. Its legacy numbered-direction rule is superseded;
  all other applicable numbering discipline remains.
- [ARCH004](../../.engloop/architecture/ARCH004_executable-core.md): prose, executable
  stage rules, an independent behavioral model, exploration, generated conformance,
  and direct tests must agree.
- [ARCH005](../../.engloop/architecture/ARCH005_component-pattern.md): generic,
...
- [Readiness is a gate](../../.engloop/learnings/cards/readiness-is-a-gate.md), sourced
  from `PM001/LEARN001`–`PM001/LEARN003`: readiness is computed from a complete inventory,
  never narrated.
- [Verification follows artifact class](../../.engloop/learnings/cards/verification-follows-artifact-class.md),
  sourced from `PM002/LEARN001`–`PM002/LEARN003`: components use unit/property evidence;
...
- [Model observable behavior, not internal shape](../../.engloop/learnings/cards/model-observable-behavior.md),
  sourced from `PM002/LEARN002` and `PM003/LEARN001`–`PM003/LEARN002`: model representative
...
- [Adequate models prove rejection](../../.engloop/learnings/cards/adequate-models-prove-rejection.md),
  sourced from `PM004/LEARN001`–`PM004/LEARN003`: require materially branching behavior and
...
The product, repository, bundle, tool, and package remain **EngLoopKit** with identifier
**`engloopkit`**. The first-party command extension identifier is **`engloop`**, because
...
## Repository memory root

V2 has exactly one tracked hidden EngLoop root: `.engloop/`. It contains the required
explicit `.engloop/config.json`, numbered engineering-loop memory, Learnings Pyramid
cards and retrieval cases, and ignored transient output under `.engloop/out/`.

There is no `.engloopkit/` directory, no live `engloop/` compatibility tree after
migration, and no lookup fallback among those paths. Root `NORTHSTAR.md` and root
`LEARNINGS.md` remain visible as the direction and learning entry points. If both the
old and new artifact roots exist, authority is ambiguous and the repository fails
validation rather than merging or selecting a root.
...
4. **Given** one or more stabilized incidents selected for review, **when** Stage 21
  ...
  `PMxxx/LEARNxxx` source learnings plus concrete `RPIxxx` repair items.
...
### User Story 5 — Guided, least-privilege custom agents (Priority: P1)

As an engineer using EngLoopKit in VS Code, I want each stage agent to expose the
right tools, prerequisites, and reviewed next-step buttons so that the picker is a
safe executable guide to the engineering loop rather than a list of generic prompts.

**Independent value:** A disposable single-root installation can validate all agent
headers and handoffs without implementing a consumer application.
...
2. **Given** any nonterminal or branching stage completes, **when** its response ends,
  **then** the supported next choices appear as handoff buttons with prefilled prompts
  and none auto-submit.
3. **Given** Stage 07 identifies a SUT, model, exploration, or valid-evidence outcome,
  **then** its four buttons route respectively to 04, 05, 06, or 08; no unrelated lane
  is offered.
4. **Given** Stage 08 produces PASS, **then** operation is authorized but no Incident,
...
6. **Given** a generated prompt file invokes one of these agents, **then** it does not
  specify `tools` and therefore cannot override the agent's least-privilege tool set.
7. **Given** agent-scoped hooks are supported and enabled, **when** the first prompt is
  submitted for an agent session, **then** the pinned EngLoopKit entry validator runs
  for that exact stage and mechanically blocks rejected entry. **Given** Preview hooks
  are unavailable or disabled, **then** the body unconditionally invokes the same
  validator before other actions, the session identifies itself as reduced-assurance,
  and trusted EngLoopKit tooling rejects every durable stage transition or evidence
  acceptance whose entry prerequisites do not pass.

## Custom-agent UX contract

Every v2 command source and generated `.agent.md` has semantically equivalent
frontmatter. Common required fields are:

```yaml
name: <exact command ID>
description: <stage-specific trigger and outcome>
...
pre-action boundary in this contract. The body must unconditionally invoke the same
versioned validator before acting, but a prompt-body instruction is not misrepresented
as a platform interceptor. Every durable stage transition and evidence-acceptance
operation independently revalidates entry through trusted EngLoopKit tooling. Thus a
hook-disabled session is supported but explicitly reduced-assurance: observed body
compliance is acceptance evidence, while only the trusted tool gate can issue accepted
state/evidence. Hook commands use no secrets or repository-editable script and validate
all input. A hook field may ship only after its exact source-to-installed preservation
and cross-platform execution are proven.

Release acceptance is pinned to VS Code Insiders `1.129.0-insider`, commit
`29d19ddd1af725baf537b6b328843bcdc2d29ba1` (build date 2026-07-09), and the tracked
agent-surface schema projection derived from official documentation retrieved
2026-07-10. Another VS Code build is unsupported until the identical P0 preservation,
schema-load, diagnostics, picker, hook, and handoff acceptance suite passes against it.

`model` is intentionally omitted from v2 agents: no stage-specific model pin has yet
demonstrated more value than respecting the user's available model selection. Adding a
future model or prioritized fallback list requires measured evidence and a governed
change. `infer` is forbidden because VS Code deprecates it in favor of
...
direction. Git is its revision history. v2 has no current numbered direction command,
artifact class, prefix, counter, template, required legacy directory, or duplicate live
direction file. Migration moves and evolves useful initial direction content into the root Northstar; deletion from
...
`chronological PMxxx/LEARNxxx sequence → living subject cards → root LEARNINGS.md → on-demand instruction/retrieval`
...
- A source identity is globally addressable as `PMxxx/LEARNxxx`; `LEARN` numbering remains
  local to its PM.
- Condensation never replaces, edits, deletes, or severs source learnings.

### Layer 2 — living subject cards

Cards live under `.engloop/learnings/cards/<subject>.md`. Each card focuses on
...
`PMxxx/LEARNxxx`, and visible conflicts, supersessions, or unresolved tension. Git retains
...
|---|---|
| EngLoop command | One exact ordered invocation surface conforming to ARCH002's loop contract. |
...
| ARCH / architecture constitution | Long-lived boundary, ownership, and contract derived from running evidence. |
| SPEC delivery set | Stage 04's accepted specification, plan, tasks, and governed implementation evidence. |
| Module | A complete inventory unit classified as generic component, pure value-type unit, or domain vertical. |
| MODEL | Human-readable and executable description of behavioral state, actions, outcomes, guards, and invariants. |
| CORD | Bounded exploration scenarios, constraints, goals, and graph/path evidence. |
...
- **FR-CMD-008:** Missing or ambiguous command identity or stage evidence MUST fail
  ...
  may mask it.
- **FR-CMD-009:** Every governed v2 repository MUST use exactly one tracked `.engloop/`
  root, with required config at `.engloop/config.json` and ignored transient output at
  `.engloop/out/`; a current `.engloopkit/` directory, live `engloop/` compatibility
  tree, or dual-root state MUST fail validation.
...
- **FR-AGT-009:** Agent hooks MUST execute reviewed versioned tooling, accept no
  ...
  or unavailable hook MUST be reported as reduced assurance, MUST NOT bypass the body
  check or trusted durable-stage gate, and MUST NOT be represented as hook-equivalent
  strict pre-action enforcement.
- **FR-AGT-010:** `infer` MUST be absent. `model` MUST be absent until a later governed
  decision demonstrates and tests a stage-specific model/fallback benefit.
- **FR-AGT-011:** If Spec Kit cannot preserve any required source frontmatter field,
  installation and release MUST fail with the dropped field and affected agent IDs;
...
- **FR-NS-005:** Current v2 artifacts MUST contain no numbered direction command,
  prefix, counter, template, required legacy directory, or duplicate live direction document.
...
- **FR-VER-003:** Generic/domain-free components MUST be extracted before behavioral
  modelling, excluded from ceremonial MODEL/CORD artifacts, and verified by unit/property
...
- **FR-OPS-003:** Stage 21 MUST analyze a selected set of stabilized incidents and
  produce class-level `PMxxx/LEARNxxx` learnings and actionable `RPIxxx` repairs.
...
- **FR-LRN-003:** Cards MUST live by subject under
  `.engloop/learnings/cards/` and satisfy the card content contract.
...
- **FR-MIG-001:** EngLoopKit, `tthp`, `engloop-workshop`, and
  ...
  usable v2 roots.
- **FR-MIG-002:** EngLoopKit MUST rename its tracked `engloop/` tree to `.engloop/`,
  evolve its current initial seed into root `NORTHSTAR.md`, remove the current seed
  file/template/counter/prefix, and retain both artifact and Northstar ancestry through
  Git.
- **FR-MIG-003:** TTHP MUST rename its tracked `engloop/` tree to `.engloop/` and move
  its tracked initial direction content into root `NORTHSTAR.md`; the old root,
  duplicate live direction, and legacy counter/template MUST not remain.
- **FR-MIG-004:** The workshop and verification consumer MUST each end with exactly one
  `.engloop/` root and one reviewed root `NORTHSTAR.md` reflecting that repository's
  own direction, with no current `engloop/`, `.engloopkit/`, or legacy numbered-direction machinery.
...
- **FR-MIG-007:** EngLoopKit's manifests, bundle/catalog/release metadata, commands,
  ...
  migrated coherently in release 1.7.0. Product version 2.x MUST NOT be selected unless
  a separate explicit maintainer decision authorizes it.
...
| Both `engloop/` and `.engloop/` exist, or `.engloopkit/` exists | Fail root validation and require an explicit clean migration; do not merge, prefer, or fall back. |
...
- Treating the internal “Ordered EngLoop v2” feature label as authority to publish any
  2.x package, bundle, extension, tool, catalog entry, tag, URL, or artifact.
...
- Deleting or rewriting accepted PM/LEARN sources during condensation.
...
- **SC-015:** In a clean-context usability check, every evaluator can identify the
  ...
  prose.
- **SC-016:** Each migrated repository has exactly one tracked `.engloop/` root and
  one `.engloop/config.json`, with 0 current `engloop/` roots, 0 `.engloopkit/`
  directories, and root-visible `NORTHSTAR.md` and `LEARNINGS.md` entry points.
- **SC-017:** A case-sensitive inspection of every current registry, code/test prefix
  set, active artifact/template filename, normative document, command body, manifest,
...
bounded by both 500 words and 60 nonblank lines. Specification and planning are
complete. Task execution begins with the blocking P0 Spec Kit/VS Code custom-agent
preservation canary; failure routes to the smallest supported upstream Spec Kit
capability change and cannot be bypassed by generated-file post-processing.
