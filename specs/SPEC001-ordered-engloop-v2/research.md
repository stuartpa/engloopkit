...
|---|---|---|
| .NET target | ELK requires modern `.slnx` support and one current runtime baseline. | Target `net10.0`, pin SDK 10.0.303, and use `EngLoopKit.slnx` for local/CI build and test. |
...
| YAML semantic validation | Required frontmatter contains nested mappings and sequences. YamlDotNet 18.1.0 is MIT-licensed and compatible with the selected runtime. | Pin YamlDotNet 18.1.0 in the domain-free document-validation component and compare parsed semantic projections; do not use line regexes or a handwritten partial YAML parser for release acceptance. |
